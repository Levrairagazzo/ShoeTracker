...
- Minimal API, no MVC controllers. Endpoints are grouped by resource in `Endpoints/ShoeEndpoints.cs`, `Endpoints/RunEndpoints.cs`, `Endpoints/AuthEndpoints.cs` and `Endpoints/StravaEndpoints.cs`, registered via extension methods (`MapShoeEndpoints`, `MapRunEndpoints`, `MapAuthEndpoints`, `MapStravaEndpoints`) called from `Program.cs`.
- Strava integration lives in `Services/Strava/`: `StravaClient` (typed `HttpClient` for Strava's OAuth endpoints), `StravaOptions` (the `Strava` config section: `ClientId`, `ClientSecret`, `RedirectUri`) and `StravaTokenStore`, the only code that should read or write `StravaConnection` tokens. It encrypts them with ASP.NET Core Data Protection and `GetValidAccessTokenAsync` refreshes them within 5 minutes of expiry. The connect flow binds Strava's `state` to the browser with a short-lived `ShoeTracker.StravaState` cookie, requires the `activity:read_all` scope, and reports its outcome by redirecting to `/?strava=<connected|denied|missing-scope|error>`; the client then starts an import. `StravaImporter` pages through `/athlete/activities` (200 per page) and stores runs with `ShoeId = null`, `Source = Strava`, the local start date, distance in km, `Type` (`Run`/`Trail`/`Treadmill`, where Strava's `trainer` flag also means treadmill), `IsRace` (`workout_type` 1) and the start coordinates. Re-imports only request activities from the day before the latest imported run, and update already-known `StravaActivityId`s in place with Strava's current values. While any imported run has `Type == null` (imported before the field existed), the import fetches the whole history instead, to backfill it. The whole import is saved at once, so a failure part-way keeps nothing. Disconnecting keeps imported runs.
- Run locations (`Services/Geocoding/`): `PlaceNameBackgroundService` names run start points via OpenStreetMap's Nominatim (`NominatimClient`), at startup, after each import (`PlaceNameSignal`) and every 5 minutes. `PlaceNameResolver` groups starts into ~1 km areas (coordinates rounded to 2 decimals) and caches each area's name in `PlaceNames`, including misses (empty name). Only the rounded area centre is sent, never the exact start point. Lookups are spaced by `Geocoding:MinInterval` (1.1 s; Nominatim allows 1 request/second and requires the identifying `User-Agent` set in `Program.cs`). Run responses carry `Location` from that cache, plus `IsUltra` (longer than a marathon, `Services/RunLabels.cs`), computed rather than stored.
- Request/response shapes are plain C# records in `Dtos/`.
- Domain entities (`Models/User.cs`, `Models/Shoe.cs`, `Models/Run.cs`) are persisted via EF Core (`Data/ShoeTrackerContext.cs`); one user has many shoes and many runs, and one shoe has many runs (all cascade on delete). A run's `ShoeId` is nullable: a run can be *unassigned* (e.g. imported from Strava before it's attributed to a shoe), so every run carries its own `UserId` and ownership is checked on `Run.UserId`, never derived through `Run.Shoe`. `Run.Source` is `Manual` or `Strava` (stored as a string), with `StravaActivityId` unique per user. Strava runs are read-only apart from their shoe: the `/shoes/{shoeId}/runs/{id}` PUT/DELETE return `409` for them (the next import would undo a local edit), and deleting a shoe unassigns its Strava runs instead of deleting them. `User.DefaultShoeId` (FK, set null when the shoe is deleted) is where newly imported Strava runs go; the user's very first import (their history) always stays unassigned.
...
- On startup, `Program.cs` migrates the database automatically — no manual migration step needed when running the API. Migration is staged around the admin seed step: a database that hasn't yet applied `AddShoeOwnership` is migrated only to `AddUsers`, the admin account is seeded, then the remaining migrations run. This is because `AddShoeOwnership` backfills pre-existing shoes to the lowest-Id user, so that user must exist first. For the same reason the seed step inserts the admin with plain SQL touching only the columns `AddUsers` created: inserting through the EF model would also write columns later migrations add to `Users` (e.g. `DefaultShoeId`) and fail on a fresh database.
...
Note: `/shoes`, `/shoes/{shoeId}/runs`, `/runs` and `/strava` endpoints require an active app-level login session (`.RequireAuthorization()`) and are scoped to the logged-in user: every query filters on the current user's id (`ClaimsPrincipal.GetUserId()` in `Endpoints/ClaimsPrincipalExtensions.cs`, read from the cookie's `NameIdentifier` claim). Another user's shoe or run returns `404`, not `403`, so its existence isn't revealed. Any new endpoint touching shoes/runs must apply the same filter — `UserScopingTests` covers every existing endpoint.
...
**Run tests** (from `tests/ShoeTracker.Api.Tests/`) — unit tests for services, plus `WebApplicationFactory`-based integration tests (`AuthEndpointsTests`, `ShoeEndpointsTests`, `RunEndpointsTests`, `StravaEndpointsTests`, `StravaImportTests`, `PlaceNameTests`, `RunAssignmentTests`, `UserScopingTests`) that run the real API against a throwaway SQLite file, and `MigrationTests` for migrations that transform existing data. New integration tests should derive from `ApiTestBase`, which sets up the app with two accounts (`OwnerEmail`, `OtherEmail`), dummy Strava credentials, and `FakeHttpHandler`s (`Strava`, `Nominatim`) standing in for strava.com and the geocoder so no test reaches a real external API (the geocoding background service is off; tests call `PlaceNameResolver` directly), and provides login/create-shoe/log-run/connect-Strava helpers:
```bash
dotnet test
```
Run a single test:
...
- For local dev, set the seed credentials via `dotnet user-secrets set "SeedAdminEmail" "..."` / `"SeedAdminPassword" "..."` from `src/ShoeTracker.Api/` (or shell env vars), and likewise `Strava:ClientId` / `Strava:ClientSecret` — never put real credentials in `appsettings.Development.json`, which is committed to git. It only holds the non-secret dev `Strava:RedirectUri` (`http://localhost:5173/api/strava/callback`, via the Vite proxy).
- Strava in Docker: `docker-compose.yml` maps `STRAVA_CLIENT_ID`, `STRAVA_CLIENT_SECRET` and `STRAVA_REDIRECT_URI` (default `https://milesleft.run/api/strava/callback`) from `.env`. Without credentials, `/strava/status` reports `available: false` and the client hides the Connect button. The Data Protection key ring (auth cookie + Strava tokens) is persisted at `DataProtection__KeysPath=/data/keys` on the `shoe-data` volume; if `DataProtection:KeysPath` is unset (e.g. `dotnet run`), ASP.NET Core's default per-user location is used.
