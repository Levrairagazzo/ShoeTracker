...
- Strava integration lives in `Services/Strava/`: `StravaClient` (typed `HttpClient` for Strava's OAuth endpoints), `StravaOptions` (the `Strava` config section: `ClientId`, `ClientSecret`, `RedirectUri`) and `StravaTokenStore`, the only code that should read or write `StravaConnection` tokens. It encrypts them with ASP.NET Core Data Protection and `GetValidAccessTokenAsync` refreshes them within 5 minutes of expiry. The connect flow binds Strava's `state` to the browser with a short-lived `ShoeTracker.StravaState` cookie, requires the `activity:read_all` scope, and reports its outcome by redirecting to `/?strava=<connected|denied|missing-scope|error>`.
...
**Run tests** (from `tests/ShoeTracker.Api.Tests/`) — unit tests for services, plus `WebApplicationFactory`-based integration tests (`AuthEndpointsTests`, `ShoeEndpointsTests`, `RunEndpointsTests`, `StravaEndpointsTests`, `UserScopingTests`) that run the real API against a throwaway SQLite file, and `MigrationTests` for migrations that transform existing data. New integration tests should derive from `ApiTestBase`, which sets up the app with two accounts (`OwnerEmail`, `OtherEmail`), dummy Strava credentials, and a `FakeStravaHandler` (`Strava`) standing in for strava.com so no test reaches the real API, and provides login/create-shoe/log-run helpers:
```bash
dotnet test
```
Run a single test:
