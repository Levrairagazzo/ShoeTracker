...
public class RunEndpointsTests : ApiTestBase
{
    ...
    public async Task Create_ReturnsCreatedRun()
    {
        ...
        Assert.Equal(new RunResponse(run.Id, new DateOnly(2026, 1, 2), 10.5, shoe.Id), run);
    }
    ...
    public async Task Update_ChangesRunAndShoeMileage()
    {
        ...
        Assert.Equal(new RunResponse(run.Id, new DateOnly(2026, 1, 5), 8, shoe.Id), await response.Content.ReadFromJsonAsync<RunResponse>());
